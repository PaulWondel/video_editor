...
class TimelineClipItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, width, height, title, file_path, track_num, duration_sec, is_audio=False):
...
class CapCutTimelineView(QGraphicsView):
    ...
    def add_clip(self, file_path, duration_sec, track_num=1, is_audio=False):
        ...
        if not is_audio:
            ...
            clip = TimelineClipItem(start_x, 25, clip_width, 30, filename, file_path, track_num, duration_sec, is_audio=False)
            self.scene.addItem(clip)
            self.video_clips.append(clip)
        else:
            start_x = sum([item.rect().width() for item in self.audio_clips])
            clip = TimelineClipItem(start_x, 85, clip_width, 30, filename, file_path, track_num, duration_sec, is_audio=True)
            self.scene.addItem(clip)
            self.audio_clips.append(clip)

        self.scene.setSceneRect(0, 0, max(1000, self.scene.itemsBoundingRect().width() + 100), 120)

    def get_ordered_video_clips(self):
        """Returns video clip items ordered by horizontal position on the timeline."""
        return sorted(self.video_clips, key=lambda clip: clip.scenePos().x() + clip.rect().x())

    def get_ordered_video_tracks(self):
        sorted_clips = self.get_ordered_video_clips()
...
class ModernMacVideoEditor(QMainWindow):
    def __init__(self):
        ...
        self.timeline_sequence = []  # List of {"path", "duration", "start_time", "end_time"}
        self.total_timeline_duration = 0.0
        self.current_clip_index = -1
        self.is_switching_clip = False
    ...
    def setup_ui(self):
        ...
        # 2. Unified Timeline Preview Monitor
        preview_group = QGroupBox("Timeline Preview Monitor")
        preview_layout = QVBoxLayout(preview_group)

        self.timeline_preview_widget = QVideoWidget()
        self.timeline_preview_widget.setStyleSheet("background-color: #000000; border-radius: 6px;")
        self.timeline_preview_widget.setMinimumHeight(280)
        self.main_player.setVideoOutput(self.timeline_preview_widget)
        preview_layout.addWidget(self.timeline_preview_widget)

        # Playback Controls & Timeline Slider
        playback_layout = QHBoxLayout()
        self.btn_play = QPushButton("▶ Play Timeline Preview")
        ...
        # 3. Interactive Sequence Timeline
        ...
        self.main_player.positionChanged.connect(self.position_changed)
        self.main_player.mediaStatusChanged.connect(self.on_media_status_changed)

    def update_timeline_composition(self):
        """Calculates global timing offsets for all video clips on the timeline."""
        ordered_clips = self.timeline_view.get_ordered_video_clips()
        self.timeline_sequence.clear()
        
        current_offset = 0.0
        for clip in ordered_clips:
            dur = clip.duration_sec
            self.timeline_sequence.append({
                "path": clip.file_path,
                "duration": dur,
                "start_time": current_offset,
                "end_time": current_offset + dur
            })
            current_offset += dur

        self.total_timeline_duration = current_offset

        if self.timeline_sequence and self.current_clip_index == -1:
            self.load_clip_to_player(0, seek_ms=0)

    def load_clip_to_player(self, index, seek_ms=0):
        if 0 <= index < len(self.timeline_sequence):
            self.is_switching_clip = True
            self.current_clip_index = index
            clip_path = self.timeline_sequence[index]["path"]
            self.main_player.setSource(QUrl.fromLocalFile(clip_path))
            if seek_ms > 0:
                self.main_player.setPosition(seek_ms)
            self.is_switching_clip = False
    ...
    def preview_edited_audio(self):
        ...
        self.btn_play.setText("▶ Play Timeline Preview")
    ...
    def toggle_play(self):
        ...
        if self.main_player.playbackState() == QMediaPlayer.PlayingState:
            ...
            if self.current_clip_index == -1 and self.timeline_sequence:
                self.load_clip_to_player(0, seek_ms=0)
            self.main_player.play()
            self.btn_play.setText("⏸ Pause")

    def on_media_status_changed(self, status):
        """Auto-advance to next timeline video when active clip finishes playing."""
        if status == QMediaPlayer.EndOfMedia and not self.is_switching_clip:
            next_idx = self.current_clip_index + 1
            if next_idx < len(self.timeline_sequence):
                self.load_clip_to_player(next_idx, seek_ms=0)
                self.main_player.play()
            else:
                self.btn_play.setText("▶ Play Timeline Preview")

    def position_changed(self, position_ms):
        """Calculates global multi-clip time and updates scrubber position."""
        if self.is_switching_clip or self.current_clip_index < 0 or not self.timeline_sequence:
            return

        current_clip = self.timeline_sequence[self.current_clip_index]
        global_time_sec = current_clip["start_time"] + (position_ms / 1000.0)

        if not self.timeline_slider.isSliderDown() and self.total_timeline_duration > 0:
            val = int((global_time_sec / self.total_timeline_duration) * 1000)
            self.timeline_slider.setValue(val)
            self.update_time_label(int(global_time_sec * 1000), int(self.total_timeline_duration * 1000))

    def set_player_position(self, slider_val):
        """Seeks accurately across any clip segment based on overall timeline time."""
        if self.total_timeline_duration <= 0 or not self.timeline_sequence:
            return

        target_global_sec = (slider_val / 1000.0) * self.total_timeline_duration

        for idx, clip in enumerate(self.timeline_sequence):
            if clip["start_time"] <= target_global_sec <= clip["end_time"]:
                local_offset_ms = int((target_global_sec - clip["start_time"]) * 1000)
                
                if idx != self.current_clip_index:
                    was_playing = (self.main_player.playbackState() == QMediaPlayer.PlayingState)
                    self.load_clip_to_player(idx, seek_ms=local_offset_ms)
                    if was_playing:
                        self.main_player.play()
                else:
                    self.main_player.setPosition(local_offset_ms)
                break
